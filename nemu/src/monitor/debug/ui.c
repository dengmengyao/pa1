...
static int cmd_x(char *args){//获取内存起始地址和扫描长度。
  if (args == NULL) {
    ...
    for (i = 0; i < num; i++) {
      printf("0x%x   ", vaddr_read(addr, 4));//以16进制形式输出连续N个数
      addr += 4;
    }
    printf("\n");
